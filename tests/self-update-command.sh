...
curl() {
    ...
    [[ "$out" == *SHA256SUMS ]] && src="$asset_dir/SHA256SUMS" || src="$asset_dir/zapret-sonar-v1.3.3.tar.gz"
    cp "$src" "$out"
}

cmd_self_update --force --version 1.3.3 >/dev/null
new_target=$(readlink "$ZF_INSTALL_ROOT/current")
[[ "$new_target" == releases/1.3.3-* ]]
[[ "$(readlink "$ZF_INSTALL_ROOT/previous")" == releases/old ]]
[[ "$(ZF_LIBRARY_MODE=0 "$ZF_INSTALL_ROOT/zapret-sonar" --version)" == 'zapret-sonar 1.3.3' ]]
cmd_self_update --force --version 1.3.3 >/dev/null
